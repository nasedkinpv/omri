...
private extension PasteManager {
    ...
    func tryAccessibilityPaste() -> Bool {
        guard let textToPaste = NSPasteboard.general.string(forType: .string),
              !textToPaste.isEmpty else { return false }
        ...
        guard AXUIElementIsAttributeSettable(axElement, kAXValueAttribute as CFString, &isSettable) == .success,
              isSettable.boolValue else {
            return false
        }
        
        return AXUIElementSetAttributeValue(axElement, kAXValueAttribute as CFString, textToPaste as CFTypeRef) == .success
    }
...
private extension PasteManager {
    ...
    func showAPIKeyAlert() {
        // Use non-intrusive notification instead of modal alert
        let notification = NSUserNotification()
        notification.title = "API Key Required"
        notification.informativeText = "Set your API key in Dictly Settings to enable AI processing."
        notification.actionButtonTitle = "Open Settings"
        notification.hasActionButton = true
        notification.soundName = nil // Silent to avoid workflow interruption
        
        // Set up notification delegate if needed
