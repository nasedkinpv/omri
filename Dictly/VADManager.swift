...
class VADManager: ObservableObject {
    ...
    init(sensitivity: Double = 0.5, minSpeechDuration: Double = 0.25, silenceTimeout: Double = 1.0) {
        ...
        print("VAD Configuration - Speech threshold: \(Float(sensitivity))")
    }
    ...
    func stopListening() {
        ...
        if isSpeechDetected && !currentSpeechBuffers.isEmpty {
            ...
            print("VAD: Stopping mid-speech, processing partial chunk (estimated \(String(format: "%.2f", estimatedDuration))s)")
    ...
    private func handleVADEvent(_ event: VadStreamEvent) {
        ...
        case .speechStart:
            ...
            currentSpeechBuffers.removeAll(keepingCapacity: true)
            print("VAD: Speech started at \(speechStartTime)s")
        ...
        case .speechEnd:
            ...
            print("VAD: Speech ended at \(speechEndTime)s (duration: \(String(format: "%.2f", speechDuration))s)")

            // Process collected speech buffers into audio chunk
            if currentSpeechBuffers.isEmpty {
                print("VAD: No audio buffers collected, skipping chunk")
            } else if speechDuration < minSpeechDuration {
                print("VAD: Speech too short (\(String(format: "%.2f", speechDuration))s < \(String(format: "%.2f", minSpeechDuration))s threshold), skipping chunk")
            } else {
    ...
    private func processCollectedSpeechBuffers(duration: Double) {
        guard !currentSpeechBuffers.isEmpty,
            ...
            print("VAD: No speech buffers or format to process")
            return
        }
        ...
        guard let floatSamples = extractFloatSamples(from: currentSpeechBuffers) else {
            print("VAD: Failed to extract Float samples from buffers")
            return
        }

        print("VAD: Generated \(floatSamples.count) Float samples (\(String(format: "%.2f", duration))s)")
    ...
    private func pcmBuffersToWavData(buffers: [AVAudioPCMBuffer], format: AVAudioFormat) -> Data? {
        ...
        do {
            ...
            print("VAD: Error converting buffers to WAV: \(error)")
            return nil
        }
    }
    ...
    func updateSensitivity(_ newSensitivity: Double) {
        print("Updating VAD sensitivity from \(sensitivity) to \(newSensitivity)")
        ...
        print("New VAD Configuration - Speech threshold: \(Float(newSensitivity))")
    ...
    func updateTimingParameters(newMinSpeechDuration: Double, newSilenceTimeout: Double) {
        print("Updating VAD timing - minSpeech: \(newMinSpeechDuration)s, silence: \(newSilenceTimeout)s")
...
class VADManager: ObservableObject {
    ...
    init(sensitivity: Double = 0.5, minSpeechDuration: Double = 0.25, silenceTimeout: Double = 1.0) {
        print("⚠️ VADManager: FluidAudio not available, using fallback implementation")
    }
